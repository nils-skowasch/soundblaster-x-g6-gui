...
class Model:
    def __init__(self):
        ...
        self.__voice_clarity_noise_reduction_level: int = 50
...
class View:
    ...
    def create(self, notebook: wx.Notebook) -> wx.Panel:
        ...
        self.__chk_noise_reduction.Bind(wx.EVT_CHECKBOX,
                                        lambda event: self.__controller.on_toggle_noise_reduction(event))
        ...
        self.__sld_noise_reduction.Bind(wx.EVT_SCROLL_CHANGED,
                                        lambda event: self.__controller.on_change_noise_reduction_level(event))
        ...
        self.__chk_echo_cancellation.Bind(wx.EVT_CHECKBOX,
                                          lambda event: self.__controller.on_toggle_acoustic_echo_cancellation(event))
        ...
        self.__chk_smart_volume.Bind(wx.EVT_CHECKBOX, lambda event: self.__controller.on_toggle_smart_volume(event))
        vbox_voice_clarity.Add(self.__chk_smart_volume, 0, wx.ALL | wx.EXPAND, 5)

        self.__chk_mic_eq = wx.CheckBox(panel, label="Mic-EQ")
        self.__chk_mic_eq.Bind(wx.EVT_CHECKBOX, lambda event: self.__controller.on_toggle_mic_equalizer(event))
        ...
        self.__cmb_mic_eq_preset.Bind(wx.EVT_COMBOBOX,
                                      lambda event: self.__controller.on_change_mic_equalizer_preset(event))
    ...
    def set_mic_recording_volume_value(self, volume: int) -> None:
        self.__sld_mic_volume.SetValue(volume)
    ...
    def set_mic_boost_value(self, boost: int) -> None:
        self.__sld_mic_boost.SetValue(boost)
    ...
    def set_mic_monitoring_volume_value(self, volume: int) -> None:
        self.__sld_mic_monitoring.SetValue(volume)
    ...
    def set_voice_clarity_noise_reduction_slider_value(self, level: int) -> None:
        self.__sld_noise_reduction.SetValue(level)
...
class Controller:
    ...
    def update_availability(self, g6_api: G6Api | None):
        ...
        if self.__g6_api is not None:
            ...
            voice_clarity_noise_reduction_available = self.__g6_api.recording_voice_clarity_level_available()
            voice_clarity_acoustic_echo_cancellation_available = (
                self.__g6_api.recording_acoustic_echo_cancellation_enabled_available()
            )
            voice_clarity_smart_volume_available = self.__g6_api.recording_smart_volume_enabled_available()
            voice_clarity_mic_equalizer_available = self.__g6_api.recording_mic_equalizer_enabled_available()
    ...
    def on_change_mic_boost(self, event) -> None:
        # get the slider value
        boost = event.GetEventObject().GetValue()
    ...
    def on_toggle_noise_reduction(self, event) -> None:
        # get the checkbox state
        enabled = bool(event.GetEventObject().GetValue())
        ...
        if self.__g6_api is not None:
            ...
            self.__g6_api.recording_voice_clarity_level(level_percent=level)

    def on_change_noise_reduction_level(self, event) -> None:
        # get the slider value
    ...
    def on_toggle_acoustic_echo_cancellation(self, event) -> None:
        # get the checkbox state
        enabled = bool(event.GetEventObject().GetValue())
        ...
        # send command to G6
        if self.__g6_api is not None:
            self.__g6_api.recording_acoustic_echo_cancellation_enabled(enable=enabled)

    def on_toggle_smart_volume(self, event) -> None:
        # get the checkbox state
        enabled = bool(event.GetEventObject().GetValue())
        ...
        # send command to G6
        if self.__g6_api is not None:
            self.__g6_api.recording_smart_volume_enabled(enable=enabled)

    def on_toggle_mic_equalizer(self, event) -> None:
        # get the checkbox state
        enabled = bool(event.GetEventObject().GetValue())
        ...
        # send command to G6
        if self.__g6_api is not None:
            self.__g6_api.recording_mic_equalizer_enabled(enable=enabled)

    def on_change_mic_equalizer_preset(self, event) -> None:
        ...
        # send command to G6
        if self.__g6_api is not None:
            self.__g6_api.recording_mic_equalizer_preset(preset=preset)
